...
szFileMask           LIKE(szFileMask)                      ! 
                     END                                   ! 
Window               WINDOW('Kwik Source Search (Open source edition)'),AT(,,600,273),FONT('Segoe UI',10,,,CHARSET:ANSI), |
...
LocationColWidth  LONG
   ...
   CODE
      ...
      INIMgr.Update('Global','OpenClarionFilesInIDE',glo:OpenClarionFilesInIDE)
...
CheckEditor ROUTINE
      IF RECORDS(EditorQueue) > 1 AND glo:PromptForEditor = TRUE
         ...
         END
      ELSE
         IF glo:szEditorCommand <> '' OR glo:OpenClarionFilesInIDE = TRUE 
            DO HandleEdit
         END
      END
HandleEdit  ROUTINE
...
j                    LONG
szExt                CSTRING(10) 
intEditingInClarion  LONG 
intL                 LONG 

   CODE
      szURL = glo:szEditorCommand
      intEditingInClarion = FALSE 

      IF glo:OpenClarionFilesInIDE = TRUE 
         oST.SetValue(szSendToFilename)
         szExt = '.' & CLIP(oST.ExtensionOnly())
         LOOP intL = 1 TO RECORDS(ClarionExtensionsQueue)
            GET(ClarionExtensionsQueue,intL) 
            IF UPPER(szExt) = UPPER(ClarionExtensionsQueue.FileExtension)
               DO GetClarionExePath
               intEditingInClarion = TRUE 
               RUN(CLIP(glo:ClarionExePath) & ' ' & szSendToFileName)
               BREAK 
            END
         END 
      END 
      
      IF intEditingInClarion = TRUE
         EXIT 
      END 
      ...
      ELSE
         ...
         END
      END
GetClarionExePath ROUTINE 

 glo:ClarionExePath = GETREG(REG_LOCAL_MACHINE,'Software\SoftVelocity\Clarion12','root')
 IF EXISTS(glo:ClarionExePath) 
 ELSE
    glo:ClarionExePath = GETREG(REG_LOCAL_MACHINE,'Software\SoftVelocity\Clarion11','root')
    IF EXISTS(glo:ClarionExePath) 
    ELSE
       glo:ClarionExePath = GETREG(REG_LOCAL_MACHINE,'Software\SoftVelocity\Clarion10','root')
       IF EXISTS(glo:ClarionExePath) 
       ELSE
          glo:ClarionExePath = GETREG(REG_LOCAL_MACHINE,'Software\SoftVelocity\Clarion9','root')
       END        
    END 
 END 
 
 IF EXISTS(glo:ClarionExePath)
    glo:ClarionExePath = CLIP(glo:ClarionExePath) & '\bin\Clarion.exe'
 END 
...
MONITOR_DEFAULTTOPRIMARY   EQUATE(00000001h)
  ...
  END
  dCompileDate = 80629
  tCompileTime = 5292936
  strBuildDate = Left(Clip(Format(80629,@d10))) & ' - ' & Left(Clip(Format(5292936,@t04)))
  dCompileDate = 80629
  tCompileTime = 5292936
   strBuildNumber = '2021.9.' & 110 + 1  ! because link comes after generate, so the # will be 1 higher. 
  strBuildDate = Left(Clip(Format(80629,@d17))) & ' - ' & Left(Clip(Format(5292936,@t04)))
  ...
  INIMgr.Fetch('Global','OpenClarionFilesInIDE',glo:OpenClarionFilesInIDE)
