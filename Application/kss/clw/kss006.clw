...
Window               WINDOW('Options'),AT(,,541,336),FONT('Segoe UI',10),DOUBLE,TILED,GRAY,HLP('UserOptions.htm'), |
  SYSTEM
                       PANEL,AT(5,5,531,328),USE(?Panel1),BEVEL(1)
  ...
  DROP(10),FORMAT('1020L(2)|M@s255@'),FROM(EditorQueue)
                       ...
                       PROMPT('Clarion &Help File'),AT(10,42),USE(?loc:szClarionHelpFile:Prompt)
                       ENTRY(@s255),AT(80,42,422,10),USE(loc:szClarionHelpFile),COLOR(COLOR:BTNFACE),READONLY,SKIP
                       BUTTON('...'),AT(505,41,11,10),USE(?LookupFile)
                       CHECK(' Identify &Location in all files'),AT(17,66,137),USE(glo:bAllExtensions)
                       GROUP('Identify Location in these File Extensions'),AT(10,56,150,271),USE(?ClarionExtensions:Group), |
  BOXED
                         LIST,AT(17,79,136,225),USE(?List),CENTER,VSCROLL,ALRT(MouseLeft2),FORMAT('80L(2)|M@s20@'), |
  ...
  'gnificant processing overhead.')
                         BUTTON('Insert'),AT(17,309,40,12),USE(?Insert),KEY(InsertKey)
                         BUTTON('Edit'),AT(59,309,40,12),USE(?Change),KEY(CtrlEnter)
                         BUTTON('Delete'),AT(102,309,40,12),USE(?Delete),KEY(DeleteKey)
                       END
                       GROUP('Properties'),AT(165,56,355,271),USE(?EditorStylesGroup),BOXED
                         LIST,AT(170,79,102,225),USE(?lbPropertyFiles),ALRT(MouseLeft2),FORMAT('125L(2)Y@s255@'),FROM(PropertyFileQueue)
                         BUTTON('Edit'),AT(188,309,40,12),USE(?cmdPropertyEditor),TIP('Edit the highlighted prop' & |
  'erty file.')
                         PROMPT('&Confirm deletions'),AT(282,82,60,10),USE(?DeleteWarningThresholdPrompt),TRN
                         LIST,AT(357,82,38,10),USE(loc:nDeleteWarningCount),DROP(3),FROM('Never|#0|Always|#1|When|#2')
                         STRING('removing at least'),AT(398,82,,10),USE(?RemovingAtLeast),HIDE
                         SPIN(@N10~ rows~),AT(460,82,55,10),USE(loc:nDeleteWarningCount,,?loc:nDeleteWarningCount:2), |
  ...
  'ely disable the warning message.')
                         BOX,AT(282,96,11,10),USE(?loc:sResultListColorBox),COLOR(00B99D7Fh),FILL(COLOR:Black),LINEWIDTH(1), |
  ROUND
                         PROMPT('&Result List'),AT(297,96),USE(?Font:Prompt),TRN
                         ENTRY(@s63),AT(357,96,145,10),USE(szFontDescription),DISABLE
                         BUTTON('...'),AT(505,96,11,10),USE(?cmdFontDialog)
                         BOX,AT(282,111,11,10),USE(?loc:sApplicationColorBox),COLOR(00B99D7Fh),FILL(00EAD1B9h),LINEWIDTH(1), |
  ROUND
                         PROMPT('&Application'),AT(297,111),USE(?loc:ApplicationColor:Prompt),TRN
                         ENTRY(@s30),AT(357,111,145,10),USE(loc:sApplicationColor),LEFT,COLOR(COLOR:White),DISABLE, |
  SKIP
                         BUTTON('...'),AT(505,111,11,10),USE(?ApplicationColorButton),TIP('Select Application Color')
                         BOX,AT(282,126,11,10),USE(?loc:sToolbarColorBox),COLOR(00B99D7Fh),FILL(00F2E4D7h),LINEWIDTH(1), |
  ROUND
                         PROMPT('&Toolbar'),AT(297,126),USE(?loc:ToolbarColor:Prompt),TRN
                         ENTRY(@s30),AT(357,126,145,10),USE(loc:sToolbarColor),LEFT,COLOR(COLOR:White),DISABLE,SKIP
                         BUTTON('...'),AT(505,126,11,10),USE(?ToolbarColorButton),TIP('Select Toolbar Color')
                         BOX,AT(282,141,11,10),USE(?loc:sSelectedBackColorBox),COLOR(00B99D7Fh),FILL(COLOR:Red),LINEWIDTH(1), |
  ROUND
                         PROMPT('&Selected'),AT(297,141),USE(?loc:SelectedBackColor:Prompt),TRN
                         ENTRY(@s30),AT(357,141,145,10),USE(loc:sSelectedBackColor),LEFT,COLOR(COLOR:White),DISABLE, |
  SKIP
                         BUTTON('...'),AT(505,141,11,10),USE(?SelectedBackColorButton),TIP('Select Selected Text' & |
  ' Background Color')
                         BOX,AT(282,156,11,10),USE(?loc:sBookmarkBackColorBox),COLOR(00B99D7Fh),FILL(COLOR:Maroon), |
  LINEWIDTH(1),ROUND
                         PROMPT('Boo&kmark'),AT(297,156),USE(?loc:BookmarkBackColor:Prompt),TRN
                         ENTRY(@s30),AT(357,156,145,10),USE(loc:sBookmarkBackColor),LEFT,COLOR(COLOR:White),DISABLE, |
  SKIP
                         BUTTON('...'),AT(505,156,11,10),USE(?BookmarkBackColorButton),TIP('Select Bookmark Back' & |
  'ground Color')
                         BUTTON,AT(170,309,14,12),USE(?cmdDefault),ICON('check-black.ico'),TIP('Set the default ' & |
  'property file')
                         BUTTON('Delete'),AT(232,309,40,12),USE(?cmdDeletePropertyFile),TIP('Delete the highligh' & |
  'ted property file.')
                         BUTTON('Official Devuna Github'),AT(282,309,,12),USE(?btnGetSource),FONT(,,,FONT:regular)
                         BUTTON('Github - This Fork'),AT(386,309,,12),USE(?btnGithubThisFork)
                       END
                       CHECK(' &Link Paths and Files with Find Text'),AT(282,154),USE(glo:SyncPathWithPattern),TIP('Recall Pat' & |
  'hs and Files when you select prior Find text from the drop list.')
                       CHECK(' Allow &Multiple Instances'),AT(282,164),USE(glo:AllowMultipleInstances)
                       CHECK(' Auto Size Result &Columns'),AT(407,164),USE(glo:AutoSizeResultColumns)
                       PROMPT('Restore &Point Timer'),AT(282,188),USE(?glo:RestorePointTimerInterval:Prompt)
                       SPIN(@n13),AT(358,188,60,10),USE(glo:RestorePointTimerInterval),RIGHT(1),STEP(1),TIP('Set to 0 t' & |
  'o disable Restore Point Feature')
                       STRING('minutes'),AT(423,188),USE(?Minutes:String)
                       GROUP('Application Function Keys'),AT(282,199,234,38),USE(?FunctionKeyBox),BOXED,TRN
                         STRING(@s80),AT(286,209,215,10),USE(loc:MinusKeyString),RIGHT,TRN
                         REGION,AT(286,209,215,10),USE(?loc:MinusKeyStringRegion),IMM
                         BUTTON('...'),AT(503,209,11,10),USE(?cmdGetMinusKey)
                         STRING(@s80),AT(286,221,215,10),USE(loc:PlusKeyString),RIGHT,TRN
                         REGION,AT(286,221,215,10),USE(?loc:PlusKeyStringRegion),IMM
                         BUTTON('...'),AT(503,221,11,10),USE(?cmdGetPlusKey)
                       END
                       GROUP,AT(282,237,234,68),USE(?CopyrightBox),BOXED,TRN
                         STRING('Kwik Source Search - Open Source Edition'),AT(314,243,171),USE(?ProductName),FONT(, |
  ,,FONT:bold),CENTER,TRN
                         STRING(@s31),AT(336,251,127),USE(glo:szVersion),CENTER,TRN
                         STRING('© Copyright  2011-2017'),AT(336,259,127),USE(?Copyright),CENTER,TRN
                         STRING('Devuna Inc.'),AT(336,268,127),USE(?CompanyName),CENTER,TRN
                         REGION,AT(359,277,90,10),USE(?rgnWebAddress),IMM
                         STRING('http://www.devuna.com'),AT(336,277,127),USE(?strWebAddress),FONT(,10,COLOR:Blue,FONT:regular), |
  CENTER,TRN
                         STRING(''),AT(329,286,141),USE(?strRegisteredTo),CENTER,HIDE,TRN
                         STRING('Devuna Inc.'),AT(336,268,127,10),USE(?CompanyName:2),CENTER,TRN
                       END
                       BUTTON('&Apply'),AT(476,309,40,12),USE(?cmdApply)
                       BUTTON('Cancel'),AT(276,268),USE(?Cancel),HIDE
                       CHECK(' Include files with binary content'),AT(282,175,142,10),USE(glo:DisableSlashP),TIP('Disable /p' & |
  ' in FINDSTR'),VALUE('1','0')
                       CHECK(' &Open Clarion Files In IDE'),AT(428,23,103,10),USE(glo:OpenClarionFilesInIDE)
                     END
...
Looped BYTE
  ...
  ReturnValue = PARENT.TakeAccepted()
    ...
    OF ?btnGithubThisFork
      ThisWindow.Update()
            szURL = 'https://github.com/mriffey/Devuna-KwikSourceSearch'
            szNull = ''
            kcr_ShellExecute(window{prop:handle},0,szURL,0,szNull,1)
    OF ?cmdGetMinusKey
      ThisWindow.Update()
